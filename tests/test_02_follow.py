...
class TestFollow:
    ...
    def test_01_make_follow(self, user_client, db_user):
        ...
        follow_list_before = user_client.get('/api/v1/follow').json()
        response = user_client.post('/api/v1/follow', data=data)
        follow_list_after = user_client.get('/api/v1/follow').json()
    ...
    def test_05_delete_follow(self, user_client, db_follow):
        username = 'test'
        follow_list_before = user_client.get('/api/v1/follow').json()
        response = user_client.delete(f'/api/v1/follow/{username}')
        follow_list_after = user_client.get('/api/v1/follow').json()
